...
def demo_SPECT_reconstruction(datatype="phantom"):
    ...
    figure(figsize=[18, 6])
    subplot(1, 2, 1);
    imshow(spect._measurement[:, :, 10], cmap='gray');
    subplot(1, 2, 2);
    imshow(spect._attenuation[64, :, :], cmap='gray');
    ...
    figure(figsize=[18, 6])
    subplot(131);
    imshow(activity.data[:, :, 64], cmap='gray', vmax=180.0);
    subplot(132);
    imshow(activity.data[:, 80, :], cmap='gray', vmax=180.0);
    subplot(133);
    imshow(activity.data[64, :, :], cmap='gray', vmax=180.0)
