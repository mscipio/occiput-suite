...
__all__ = ["DEFAULT_PROJECTION_PARAMETERS", "DEFAULT_BACKPROJECTION_PARAMETERS", "ProjectionParameters",
           "BackprojectionParameters",
           "PET_project_compressed", "PET_backproject_compressed", "has_NiftyPy"]

from ...Core.Errors import *
from ...Core.NiftyPy_wrap import PET_project_compressed, PET_backproject_compressed, has_NiftyPy
        ...
        if parameters is None:
            ...
            if len(parameters) == len(self.default_parameters.keys()):
                ...
                raise UnknownParameter(
                    'Parameter %s specified for ProjectionParameters is not compatible. ' % str(parameters))
        else:
            raise UnknownParameter(
                'Parameter %s specified for ProjectionParameters is not compatible. ' % str(parameters))

    def load_from_dictionary(self, dictionary):
        self.N_samples = dictionary['N_samples']
        # Number of samples along a line when computing line integrals
        self.sample_step = dictionary['sample_step']
        # distance between consecutive points along a line when computing line integrals (this is in the same unit
        # measure as the size of the imaging volume (activity_size and attenuation_size))
        self.background_activity = dictionary['background_activity']
        # Activity in voxels outside of the imaging volume
        self.background_attenuation = dictionary['background_attenuation']
        # Attenuation in voxels outside of the imaging volume
        self.truncate_negative_values = dictionary['truncate_negative_values']
        # If set to 1, eventual negative values obtained when projecting are set to
        # 0 (This is meant to remove eventual unwanted small negative values due to
        # FFT-based smoothing within the projection algorithm - note that numerical
        # errors may produce small negative numbers when doing FFT-IFFT even if the
        ...
        # Direction parameter for the projector and b-proj.it affects performance only; 2 and 5 usual best values.
        self.block_size = dictionary['block_size']
        # Number of blocks per thread for the GPU accelerated projector and back-proj. It affects performance only.
        # Typical values: 256, 512, 768
