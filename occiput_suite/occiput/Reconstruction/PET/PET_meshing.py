...
class Michelogram:
    def __init__(self, n_rings=64, span=DEFAULT_SPAN, max_ring_difference=60):
        ...
        if not self._verify_max_ring_difference(span, n_rings, max_ring_difference):
            max_ring_difference_actual = self._largest_compatible_max_ring_difference(span, n_rings,
                                                                                      max_ring_difference)
            ...
            if compatible_max_ring_diff is []:
                ...
                print(
                    "Michelogram: 'maximum_ring_difference' %d is not compatible with 'span' %d. Compatible values are " % (
                        max_ring_difference, span) + str(
                        compatible_max_ring_diff) + ". \nUsing %d" % max_ring_difference_actual)
