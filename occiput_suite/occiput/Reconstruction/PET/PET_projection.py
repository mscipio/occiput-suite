...
from ...Core.NiftyPy_wrap import PET_compress_projection, PET_uncompress_projection, \
...
from ...Visualization.Visualization import ipy_table, has_ipy_table, svgwrite, has_svgwrite, ProgressBar, TriplanarView
...
def display_PET_Projection_geometry():
    filename = inspect.getfile(__occiput_suite.occiput).strip("__init__.pyc") + "Data" + os.sep + "occiput_ray_tracer_PET.png"
...
class Binning():
    ...
    def __init__(self, parameters=None):
        ...
        if parameters is None:
            ...
            raise UnknownParameter(
                'Parameter %s specified for the construction of Binning is not compatible. ' % str(parameters))
    ...
    def _repr_html_(self):  # FIXME: add angles_axial and angles_azimuthal
        ...
        for i in range(len(self.angles_azimuthal) // N_per_row + 1):
            ...
            if i == 0:
                table_data.append(["Angles_azimuthal [deg]",
                                   array_to_string(rad_to_deg(self.angles_azimuthal[i_start:i_end]), "%3.4f ")])
            else:
                table_data.append(["", array_to_string(rad_to_deg(self.angles_azimuthal[i_start:i_end]), "%3.4f ")])
        i_start = 0
        i_end = len(self.angles_axial) - 1
        if i_end - i_start >= 4:
            table_data.append(["Angles_axial [deg]", str(rad_to_deg(self.angles_axial[i_start])) + " " + str(
                rad_to_deg(self.angles_axial[i_start + 1])) + "  ...  " + str(
                rad_to_deg(self.angles_axial[i_end - 1])) + "   " + str(rad_to_deg(self.angles_axial[i_end])), ])
    ...
    def _make_svg(self):
        ...
        # line = dwg.add(dwg.polyline([(10, 10), (10, 100), (100, 100), (100, 10), (10, 10)],
...
class PET_Projection_Sparsity():
    """Represents the sparsity pattern of projection data and implements methods to compress and uncompress
    data according to the sparsity pattern of self.
    If offsets and locations are not specified as parameters of the constructor,
    the data structure represents dense projection data. """
    ...
    def compress_data(self, data):
        """Compresses an array of uncompressed projection data,
        according to the sparsity pattern of self (self.offsets and self.locations). """
    ...
    def uncompress_data(self, data):
        """Uncompresses a data array that contains projection compressed
        according to the sparsity pattern of self (self.offsets and self.locations). """
    ...
    def same_sparsity_as(self, obj):
        """Returns True if the argument is an object of type PET_Projection_Sparsity
        with the same identical sparsity pattern as self. """
    ...
    def get_projection_info(self):
        self.sparsity_info = {'offsets': self.offsets,
    ...
    def get_subset(self, subset_matrix):
        ...
        N_locations_sub = int32(
            self.N_u * self.N_v * N_subsets)  # FIXME Create C function to compute N_locations_sub for compr data
        ...
        sparsity_sub = PET_Projection_Sparsity(N_subsets, 1, self.N_u, self.N_v, offsets=offsets_sub,
                                               locations=locations_sub)
        return sparsity_sub
...
class PET_Projection():
    ...
    def get_sparsity(self):
        """Returns an instance of PET_Projection_Sparsity,
        which describes the sparsity pattern of the projection data. """
    ...
    def compress_as_self(self, projection):
        ...
        return PET_Projection(self.get_binning(), compressed_data, self.sparsity.offsets, self.sparsity.locations,
                              self.get_time_bins())
    ...
    def compress_as(self, projection):
        ...
        return PET_Projection(projection.get_binning(), data, projection.sparsity.offsets,
                              projection.sparsity.locations, projection.get_time_bins())
    ...
    def uncompress_self(self):
        """Returns an instance of PET_Projection obtained by zero-filling self.
        The projection object returned is not sparse. """
        if self.is_uncompressed():
            return self
        uncompressed_data = self.sparsity.uncompress_data(self.data)
        return PET_Projection(self.get_binning(), uncompressed_data, offsets=None, locations=None,
    ...
    def get_compression_ratio(self):
        return (1.0 * self.get_N_locations()) / (
            self.sparsity.N_u * self.sparsity.N_v * self.sparsity.N_axial * self.sparsity.N_azimuthal)
    ...
    def get_projection_info(self):
        ...
        self.prompts_sparsity_info = self.sparsity.get_projection_info()
        self.prompts_sparsity_info['compress_ratio'] = self.get_compression_ratio()
        return self.prompts_sparsity_info
    ...
    def to_nd_array(self):
        ...
        if not self.is_uncompressed():
            ...
            uncompressed = self
        shape = (uncompressed.sparsity.N_axial, uncompressed.sparsity.N_azimuthal, uncompressed.sparsity.N_u,
    ...
    #TODO: test for optimal orientation to show projection
    def display(self,clim=None, cmap=None, figsize = (20,9)):
        V = TriplanarView(self)
    ...
    def _display_legacy(self, axial=True, azimuthal=None, index=0, scale=None, open_browser=False):
        ...
        if axial is None and azimuthal is None:  # if axial and azimuthal are not specified,
            # select them automatically based on data shape
    ...
    def apply_noise_Poisson(self):  # TODO add function to apply NB (negative binomial) noise to data
        self.data = float32(poisson(self.data))
    ...
    def get_subset(self, subsets_matrix):
        ...
        N_locations_sub = int32(
            self.binning.N_u * self.binning.N_v * N_subsets)  # FIXME Create C function to compute N_locations_sub for compr data
        data_sub, offsets_sub, locations_sub = PET_get_subset_projection_array(self.data, subsets_matrix,
