...
class Michelogram:
    ...
    def _make(self):
        ...
        for i in range(n_sinograms_per_side - 1):
            n_planes[2 + i] = n_planes[1 + i] - 2 * self.span
        self.n_sinograms = n_sinograms
        self.n_planes = n_planes

        self.michelogram_sinogram = -1 * ones([self.n_rings, self.n_rings], order='F', dtype=int32)
        self.michelogram_plane = -1 * ones([self.n_rings, self.n_rings], order='F', dtype=int32)
    ...
    @staticmethod
    def _verify_max_ring_difference():
        return True
