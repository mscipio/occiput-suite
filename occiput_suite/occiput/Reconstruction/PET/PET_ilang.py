...
class PET_Static_Poisson(Model):
    ...
    def log_conditional_probability_gradient_lambda(self, lambda_):
        ...
        Ny = lambda_.shape[1]
        Nz = lambda_.shape[2]
        gradient = self.PET_scan.backproject(self.PET_scan.get_measurement()[0] / (projection_data[0] + self.EPS), Nx,
                                             Ny, Nz, alpha)
        return gradient
...
class PET_Dynamic_Poisson(Model):
    variables = {'lambda': 'continuous', 'alpha': 'continuous', 'roi_1': 'continuous', 'roi_1': 'continuous',
                 'counts_1': 'discrete', 'counts_2': 'discrete'}
    dependencies = [['lambda', 'counts_1', 'directed'], ['lambda', 'counts_2', 'directed'],
                    ['alpha', 'counts_1', 'directed'], ['alpha', 'counts_2', 'directed'],
                    ['roi_1', 'counts_1', 'directed'], ['roi_2', 'counts_2', 'directed']]
