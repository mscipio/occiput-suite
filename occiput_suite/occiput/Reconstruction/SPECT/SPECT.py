...
from numpy import float32, float64, int32, uint32, pi, ones, zeros, isscalar, linspace, ndarray, where, exp, log, \
    arange, newaxis, asarray, ascontiguousarray, isinf
...
from ...Core.NiftyPy_wrap import SPECT_project_parallelholes, SPECT_backproject_parallelholes
from ...DataSources.FileSources.Volume import import_nifti
from . import Collimators
from . import Scintillators
from ...Visualization.Visualization import ProgressBar, svgwrite, has_svgwrite
...
class SPECT_Projection():
    ...
    def display_in_browser(self, axial=True, azimuthal=False, index=0, scale=None):
        self.display(scale=scale, open_browser=True)
...
class SPECT_Static_Scan(object):
    ...
    def get_gantry_angular_positions(self):
        return (
            self._p_gantry_angular_position_first, self._p_gantry_angular_position_last,
    ...
    def get_camera_positions(self):
        return float32(linspace(deg_to_rad(self._p_gantry_angular_position_first),
                                deg_to_rad(self._p_gantry_angular_position_last),
                                self._p_gantry_angular_positions).reshape((self._p_gantry_angular_positions, 1)))
    ...
    def project(self, activity, attenuation=None, cameras=None, psf=None, subsets_array=None):
        ...
        if psf is None:
            psf = self._psf
        proj = SPECT_project_parallelholes(activity, cameras, attenuation, psf, self._background_activity,
                                           self._background_attenuation, self._use_gpu, self._truncate_negative)
    ...
    def backproject(self, projection, attenuation=None, cameras=None, psf=None, subsets_array=None):
        ...
        if psf is None:
            psf = self._psf
        backproj = SPECT_backproject_parallelholes(projection, cameras, attenuation, psf, self._background_activity,
                                                   self._background_attenuation, self._use_gpu, self._truncate_negative)
    ...
    def __make_probabilistic_graphical_model(self):
        pass
        from ...Visualization.Visualization import Graph
        self.graph = Graph({'nodes': [{'name': 'activity', 'type': 0}, {'name': 'counts', 'type': 0}],
    ...
    def _compute_normalisation(self):
        ...
        self._norm = self.backproject(
            ones((self._p_n_pix_x, self._p_n_pix_y, self._p_gantry_angular_positions), dtype=float32, order="F")).data
    ...
    def estimate_activity(self, attenuation=None, psf=None, iterations=DEFAULT_ITERATIONS,
                          subset_size=DEFAULT_SUBSET_SIZE, subset_mode='random', method="EM", m=32, factr=0.01,
                          pgtol=1e-16, maxfun=10000, smoothing=0.0, activity=None):
        progress_bar = ProgressBar()
        ...
        if method == "EM":
            ...
            for i in range(iterations):
                ...
                if subsets_array is not None:
                    ...
                    norm = self.backproject(
                        ones((self._p_n_pix_x, self._p_n_pix_y, subset_size), dtype=float32, order="F"),
                        attenuation=attenuation, psf=psf, subsets_array=subsets_array).data
                    update = (self.backproject(P, attenuation=attenuation, psf=psf,
            ...
            activity_rec, f, d = optimize.fmin_l_bfgs_b(self.get_likelihood, activity0,
                                                        fprime=self.get_gradient_activity, m=m, factr=factr,
                                                        pgtol=pgtol, args=args, maxfun=maxfun, iprint=0, bounds=bounds)
    ...
    def set_measurement(self, measurement):
        if not (self._p_n_pix_x == measurement.shape[0] and self._p_n_pix_y == measurement.shape[
            1] and self._p_gantry_angular_positions == measurement.shape[2]):
