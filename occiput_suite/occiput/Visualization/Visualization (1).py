...
class MultipleVolumes():
    def __init__(self, volumes, axis=0, shrink=256, rotate=90, subsample_slices=None, scales=None, open_browser=None):
        ...
        self._progress_bar = ProgressBar(height='6', width='100', background_color=C.LIGHT_GRAY,
    ...
    def export_image(self, volume_index, slice_index, axis=0, normalise=True, scale=None, shrink=None, rotate=0,
                     global_scale=True):
        # FIXME: handle 4D, 5D, ..
        ...
        if axis == 0:
            a = numpy.float32(self.get_data(volume_index)[slice_index, :, :].reshape(
    ...
    def display(self, axis=None, shrink=None, rotate=None, subsample_slices=None, scales=None, open_browser=None):
        ...
        self._progress_bar = ProgressBar(height='6px', width='100%%', background_color=C.LIGHT_GRAY,
                                         foreground_color=C.GRAY)
...
class VolumeRenderer():
    def __init__(self, volume, theta_min_deg=0.0, theta_max_deg=360.0, n_positions=180, truncate_negative=False,
                 psf=None, attenuation=None, scale=1.0):
        self.volume = volume
        self.psf = psf
    ...
    def __make_cameras(self, axis, direction):
        ...
        self.cameras[:, direction] = numpy.linspace(deg_to_rad(self.theta_min_deg), deg_to_rad(self.theta_max_deg),
                                                    self.n_positions)
    ...
    def render(self, axis=(90, 0, 0), direction=0, max_n_points=None):
        ...
        if has_NiftyPy:
            proj_data = projection(volume, self.cameras, self.attenuation, self.psf, 0.0, 0.0, self.use_gpu,
