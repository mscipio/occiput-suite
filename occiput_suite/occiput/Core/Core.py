...
from .NiftyPy_wrap import transform_grid, grid_from_box_and_affine, resample_image_on_grid, INTERPOLATION_LINEAR
from ..Visualization.Visualization import MultipleVolumes, VolumeRenderer, ipy_table, has_ipy_table, ProgressBar, TriplanarView
...
class Transform_Affine(object):
    ...
    def left_multiply(self, obj):
        ...
        if not self.can_left_multiply(obj):
            if hasattr(obj, 'map_from'):
                ...
                X = "X"
            print("Affine is incompatible with the given object: composing [%s,%s] with [%s,%s] is not allowed. " % (
    ...
    def set_data(self, data):
        if not isinstance(data, numpy.ndarray):
            if data is None:
                ...
                return
        # TODO: check after allowing Transformation matrix with 5x5 shape
        if not data.size == 16:
            print("Wrong size for affin matrix in numpy.ndarray format")
            return
        self.__data = data
    ...
    def _repr_html_(self):
        ...
        table_data = [[s, '', '', '', '', '', ''],
                      ...
                      [self.map_from, self.map_to, str(self.determinant()), self.is_rigid(), self.is_6DOF(),
                       self.is_rotation(), self.is_translation()], ]
        ...
        s = '<div><style>table {float: left; margin-right:10px;}</style> %s %s %s </div>' % (
            table._repr_html_(), table3._repr_html_(), table2._repr_html_())
        return s
...
class GridND(object):
    ...
    def _repr_html_(self):
        ...
        table_data = [[s, '', '', '', '', '', ''],
                      ...
                      [str(self.space), pretty(self.shape), pretty(self.min()), pretty(self.max()), pretty(self.span()),
                       pretty(self.center()), pretty(self.mean_dist_from_center())], ]
        table = ipy_table.make_table(table_data)
        table = ipy_table.apply_theme('basic')
        table = ipy_table.set_cell_style(0, 0, column_span=7)
        table = ipy_table.set_cell_style(0, 0, align='center')
