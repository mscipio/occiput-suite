...
# FIXME: perhaps get rid of this file and place code in occiput.
# Transformation (mostly), although as it is now, all imports from
# NiftyPy are in one place (here).
# This is somewhat convenient to keep track of things.
...
try:
    ...
    from ...NiftyPy.NiftyRec import TR_resample_grid \
        as __TR_resample_grid
    from ...NiftyPy.NiftyRec import TR_grid_from_box_and_affine \
        as __TR_grid_from_box_and_affine
    from ...NiftyPy.NiftyRec import TR_transform_grid \
        as __TR_transform_grid
    from ...NiftyPy.NiftyRec import PET_project_compressed, \
        PET_backproject_compressed
    from ...NiftyPy.NiftyRec import PET_compress_projection, \
        PET_uncompress_projection, PET_initialize_compression_structure
    from ...NiftyPy.NiftyRec import PET_compress_projection_array, \
        PET_get_subset_sparsity, PET_get_subset_projection_array
    from ...NiftyPy.NiftyRec import SPECT_project_parallelholes, \
        SPECT_backproject_parallelholes
    from ...NiftyPy.NiftyRec import gpu_set, gpu_reset, \
        gpu_list, gpu_exists

except BaseException:
    print("NiftyPy could not be loaded: "
          "it will not be possible to reconstruct the PET data. ")
    has_NiftyPy = False
...
def __make_grid(data, space, ndim):
    if ndim == 3:
        return _occiput_suite.occiput.Core.Grid3D(data, space)
    else:
        return _occiput_suite.occiput.Core.GridND(data, space)
...
def resample_image_on_grid(
    ...
    if verify_mapping:
        ...
        # 1) if affine_grid_to_world is not defined, verify if image and grid
        # are compatible
        if affine_grid_to_world is None:
            if not image.affine.can_inverse_left_multiply(grid):
                raise ("grid and image are not compatible. ")
        # 2) if affine_grid_to_world is defined, verify if image, grid and
        # affine_grid_to_world are compatible
        else:
            # a) check mapping from grid to world
            if not affine_grid_to_world.can_left_multiply(grid):
                raise ("grid and affine_grid_to_world are not compatible. ")
            # b) check mapping from image to world
            if not image.affine.can_inverse_left_multiply(
                    affine_grid_to_world):
                raise ("image and affine_grid_to_world are not compatible. ")
    # compute affine:
    if affine_grid_to_world is None:
        affine = image.affine
    else:
    ...
    if interpolation_mode is None:
        if image.is_mask():
            ...
            # resample:
    resampled_data = __TR_resample_grid(
        __np.float32(
            image.data), __np.float32(
            grid.data), __np.float32(
                affine.data), background, use_gpu, interpolation_mode)
