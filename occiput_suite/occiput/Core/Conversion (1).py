...
def nipy_to_occiput(nib):
    if nib.ndim == 3:
        im = __occiput_suite.occiput.Core.Core.Image3D(data=nib.get_data(), affine=nib.affine, space="world")
    else:
        im = __occiput_suite.occiput.Core.Core.ImageND(data=nib.get_data(), affine=nib.affine, space="world")
    return im
...
def occiput_from_array(array):
    if array.ndim == 3:
        im = __occiput_suite.occiput.Core.Core.Image3D(data=array, space="world")
    else:
