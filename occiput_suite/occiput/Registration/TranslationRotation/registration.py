...
from ...Core import Transform_Translation
from ...Visualization.Visualization import MultipleVolumesNiftyPy
...
class Registration_Two_Images(object):
    def __init__(self, source=None, target=None, degrees_of_freedom=3, sigma=DEFAULT_SIGMA,
    ...
    def __make_graph(self):
        ...
        self.ilang_graph.add_dependence(self.ilang_model, {'source': 'source', 'target': 'target', 'sigma': 'sigma',
    ...
    def register(self, optimization_method='GradientAscent', iterations=DEFAULT_N_ITER, n_points=DEFAULT_N_POINTS):
        ...
        self.__transformation = self.ilang_graph.get_node_value('transformation')
...
class Registration_Longitudinal():
    ...
    def __make_graph(self):
        ...
        for i in range(len(self.__images) - 1):
            ...
            self.ilang_graph.add_dependence(model, {'sigma': 'sigma_%d' % i, 'transformation': 'T_%d' % i,
                                                    'source': 'im_%d' % i, 'target': 'im_%d' % (i + 1)})
        self.ilang_sampler = Sampler(self.ilang_graph)
...
class Registration_N_Images():
    ...
    def __make_graph(self):
        ...
        for i in range(len(self.__images)):
            ...
            model = SSD_ilang('SSD_%d' % i)
            self.ilang_graph.add_dependence(model, {'sigma': 'sigma_%d' % i, 'transformation': 'T_%d' % i,
                                                    'source': 'im_%d' % i, 'target': 'template'})
        self.ilang_sampler = Sampler(self.ilang_graph)
