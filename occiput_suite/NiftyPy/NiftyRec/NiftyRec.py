...
from ...simplewrap import localpath, filepath, call_c_function, find_c_library, NOT_FOUND, FOUND, FOUND_NOT_LOADABLE, load_c_library
...
def gpu_list():
    ...
    if not r.status == status_success():
        raise ErrorInCFunction("The execution of 'gpu_list' was unsuccessful.", r.status,
...
def gpu_reset():
    ...
    if not r.status == status_success():
        raise ErrorInCFunction("The execution of 'gpu_reset' was unsuccessful.", r.status,
...
def PET_project_compressed(activity, attenuation, offsets, locations, active,
    ...
    descriptor = [{'name': 'projection', 'type': 'array', 'value': None, 'dtype': float32, 'size': (N_locations)},
                  ...
                  {'name': 'time_profiling', 'type': 'array', 'value': None, 'dtype': float32, 'size': (7)},
                  ]
    ...
    timing_dict = {"T0_transfer_to_gpu": T[0], "T1_alloc": T[1], "T2_rotate": T[2],
                   "T3_resample": T[3], "T4_integral": T[4], "T5_transfer_to_host": T[5], "T6_free": T[6], }
...
def PET_backproject_compressed(projection_data, attenuation, offsets, locations, active,
    ...
    descriptor = [{'name': 'back_projection', 'type': 'array', 'value': None, 'dtype': float32,
                  ...
                  {'name': 'direction', 'type': 'uint', 'value': direction},
                  {'name': 'block_size', 'type': 'uint', 'value': block_size},
                  {'name': 'time_profiling', 'type': 'array', 'value': None, 'dtype': float32, 'size': (10)},
                  ]
...
def PET_initialize_compression_structure(N_axial, N_azimuthal, N_u, N_v):
    ...
    descriptor = [{'name': 'N_axial', 'type': 'uint', 'value': N_axial},
                  ...
                  {'name': 'N_u', 'type': 'uint', 'value': N_u},
                  {'name': 'N_v', 'type': 'uint', 'value': N_v},
                  {'name': 'offsets', 'type': 'array', 'value': None, 'dtype': int32, 'size': (N_azimuthal, N_axial),
                   'order': 'F'},
                  {'name': 'locations', 'type': 'array', 'value': None, 'dtype': uint16,
                   'size': (3, N_u * N_v * N_axial * N_azimuthal), 'order': 'F'},
                  ]

    r = call_c_function(niftyrec_c.PET_initialize_compression_structure, descriptor)
    if not r.status == status_success():
        raise ErrorInCFunction("The execution of 'PET_initialize_compression_structure' was unsuccessful.", r.status,
...
def PET_compress_projection(offsets, data, locations, N_u, N_v):
    ...
    descriptor = [{'name': 'N_locations', 'type': 'uint', 'value': N_locations},
                  ...
                  {'name': 'locations', 'type': 'array', 'value': locations},
                  {'name': 'projection', 'type': 'array', 'value': None, 'dtype': float32, 'size': (N_locations,),
                   'order': 'F'},
                  ]
    r = call_c_function(niftyrec_c.PET_compress_projection, descriptor)
    if not r.status == status_success():
        raise ErrorInCFunction("The execution of 'PET_compress_projection' was unsuccessful.", r.status,
...
def PET_uncompress_projection(offsets, data, locations, N_u, N_v):
    ...
    if not r.status == status_success():
        raise ErrorInCFunction("The execution of 'PET_uncompress_projection' was unsuccessful.", r.status,
...
def PET_compress_projection_array(data, N_ax, N_az, N_u, N_v, threshold=1e-15):
    ...
    descriptor = [{'name': 'N_axial', 'type': 'uint', 'value': N_ax},
                  ...
                  {'name': 'active', 'type': 'array', 'value': active},
                  {'name': 'data', 'type': 'array', 'value': data},
                  {'name': 'offsets', 'type': 'array', 'value': None, 'dtype': int32, 'size': offsets_size,
                   'order': 'F'},
                  {'name': 'locations', 'type': 'array', 'value': None, 'dtype': uint16, 'size': locations_size,
                   'order': 'F'},
                  {'name': 'compressed', 'type': 'array', 'value': None, 'dtype': float32, 'size': data_size,
                   'order': 'F'},
                  ]
    r = call_c_function(niftyrec_c.PET_compress_projection_array, descriptor)
    if not r.status == status_success():
        raise ErrorInCFunction("The execution of 'PET_compress_projection_array' was unsuccessful.", r.status,
                               'niftyrec_c.PET_compress_projection_array')
    return r.dictionary['compressed'], r.dictionary['offsets'], r.dictionary['locations']


def PET_get_subset_sparsity(offsets, locations, subsets_matrix, N_u, N_v, N_locations, N_locations_sub):
    ...
    descriptor = [{'name': 'N_axial', 'type': 'uint', 'value': N_ax},
                  ...
                  {'name': 'offsets', 'type': 'array', 'value': offsets},
                  {'name': 'locations', 'type': 'array', 'value': locations},
                  {'name': 'subsets_matrix', 'type': 'array', 'value': subsets_matrix},
                  {'name': 'offsets_sub', 'type': 'array', 'value': None, 'dtype': int32, 'size': offsets_size,
                   'order': 'F'},
                  {'name': 'locations_sub', 'type': 'array', 'value': None, 'dtype': uint16, 'size': locations_size,
                   'order': 'F'},
                  ]
    r = call_c_function(niftyrec_c.PET_get_subset_sparsity, descriptor)
    if not r.status == status_success():
        raise ErrorInCFunction("The execution of 'PET_get_subset_sparsity' was unsuccessful.", r.status,
                               'niftyrec_c.PET_get_subset_sparsity')
    return r.dictionary['offsets_sub'], r.dictionary['locations_sub']


def PET_get_subset_projection_array(data, subsets_matrix, offsets, locations, N_u, N_v, N_locations, N_locations_sub):
    ...
    descriptor = [{'name': 'N_axial', 'type': 'uint', 'value': N_ax},
                  ...
                  {'name': 'locations_sub', 'type': 'array', 'value': None, 'dtype': uint16, 'size': locations_size,
                   'order': 'F'},
                  {'name': 'subsets_matrix', 'type': 'array', 'value': subsets_matrix},
                  {'name': 'data_sub', 'type': 'array', 'value': None, 'dtype': float32, 'size': data_size,
                   'order': 'F'},
                  ]
    r = call_c_function(niftyrec_c.PET_get_subset_projection_array, descriptor)
    if not r.status == status_success():
        raise ErrorInCFunction("The execution of 'PET_get_subset_projection_array' was unsuccessful.", r.status,
                               'niftyrec_c.PET_get_subset_projection_array')
    return r.dictionary['data_sub'], r.dictionary['offsets_sub'], r.dictionary['locations_sub']
...
def TR_grid_from_box_and_affine(box_min, box_max, box_n, affine_box2grid=None):
    ...
    if affine_box2grid is None:
        affine_box2grid = eye(4, dtype=float32)
    descriptor = [
...
def TR_resample_grid(image_array, grid_array, affine_index2grid=None, background=0.0, use_gpu=1,
    ...
    if affine_index2grid is None:
        affine_index2grid = eye(4, dtype=float32)
    resampled_shape = asarray([grid_array.shape[0], grid_array.shape[1], grid_array.shape[2]])
    descriptor = [{'name': 'resampled_array', 'type': 'array', 'value': None, 'dtype': float32,
                  ...
                  {'name': 'interpolation_mode', 'type': 'uint', 'value': uint32(interpolation_mode)},
                  ]
...
def TR_transform_grid(grid_array, affine_from_grid, use_gpu=1):
    ...
    if affine_from_grid is None:
        affine_from_grid = eye(4, dtype=float32)
